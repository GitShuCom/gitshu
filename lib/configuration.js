...
// `highlight` 插件会把代码中需要转义的字符提前转义，影响后续 cheerio 调整 html 内容
// `search` 插件对于体积大的书籍是个灾难。好像对中文意义不大
    ...
    .then(function() {
        if (!that.book.isSubBook()) {
            if (!version.satisfies(that.options.gitbook)) {
                throw new Error('GitShu version doesn\'t satisfy version required by the book: '+that.options.gitbook);
            }
